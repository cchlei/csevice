...
define(function(require){
    ...
    var Controller = function(input, option){
        this.option = $.extend(false, {
            ...
            'ajaxDataType': 'json',                         // string 'json' or 'xml', jsonp
    ...
    var _ajaxSend = function(keyword){
        ...
        var that = this,
            ...
            ajaxOption = {
                ...
                'success': function(theData, textStatus, jqXHR){
                    var newdata = that.option.onRec(theData);
                    if(newdata){
                        theData = newdata;
                    }
                    data = theData;
        ...
        if ($.isPlainObject(this.option.ajaxParams)) {
            ajaxOption['data'] = $.extend(false, {'query': keyword}, this.option.ajaxParams);
        } else if ($.isFunction(this.option.ajaxParams)) {
            ajaxOption['data'] = $.extend(false, {'query': keyword}, this.option.ajaxParams.apply(this, [keyword]));
        } else if (typeof(this.option.ajaxParams) === 'string') {
            ajaxOption['data'] = "query=" + keyword + "&" + this.option.ajaxParams;
        } else {
