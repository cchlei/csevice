...
define(function (require) {
    ...
    /**
     * 重置某个对象，也可用来潜克隆
     * @param reset_val     全局默认值
     * @param target        要重制的对象
     * @param def           设置每个字段的默认值
     * @returns target
     */
    ctool.reset = function(reset_val,target,def){
        def = def||{};
     ...
     * 监控object和之前发生的变化
     */
    ctool.object_diff = function(delay,target,step){
        ...
        !function(){
            ...
            rec(old,t);
            ctool.reset(old,target);
            ctool.run_until(delay,function(){
                pick_diff(target,old,result);
                rec(old,t);

                if(ctool.obj_leng(result))
                    step(result);
            })
            ...
            /**
             * 以对象1为基准，跳转对象2与1不同的字段，写入result
             * @param result  如果传该值，将新建一个对象返回
             */
            function pick_diff(obj1,obj2,result){
